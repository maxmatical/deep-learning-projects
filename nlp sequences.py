...
encoded_seq = t.texts_to_sequences(comments)
...
#train val split
xt, xv, yt, yv = train_test_split(encoded_seq, y1, test_size = 0.2)
...
model = Sequential()
model.add(Embedding(max_words, 128, input_length=xt.shape[1]))
...
model = Sequential()
model.add(Embedding(max_words, 128, input_length=xt.shape[1]))
model.add(Conv1D(256, 5,  kernel_initializer='glorot_normal',
